  ...
  const updateLineItem = (index: number, field: string, value: any) => {
    ...
    if (field === "itemId") {
      ...
      if (selectedItem) {
        ...
        item.rate = Number(selectedItem.purchasePrice);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">Create Purchase Bill</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-card border border-border/50 rounded-xl p-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="space-y-2 md:col-span-2">
              <Label htmlFor="party">Supplier / Party (Optional)</Label>
              <Select value={partyId} onValueChange={setPartyId}>
                <SelectTrigger>
                  <SelectValue placeholder="Select a party or leave empty for cash purchase" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Cash Purchase</SelectItem>
                  {parties.map(p => (
        ...
        <div className="bg-card border border-border/50 rounded-xl p-6 space-y-4">
          ...
          <Button type="button" variant="outline" onClick={addLineItem} className="mt-2 text-blue-600 border-blue-600 hover:bg-blue-50">
