...
import { ArrowLeft, User, Calendar, DollarSign, Save, Plus, Trash2, Package, Percent } from "lucide-react";
...
export default function NewQuotationPage() {
  ...
  const discountAmount = (totalAmount * formData.discount) / 100;
  const taxAmount = ((totalAmount - discountAmount) * formData.tax) / 100;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await createQuotation(activeProfileId!, {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Create a new quotation for customer</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-card border border-border/50 rounded-xl p-6 space-y-4">
            ...
            {formData.items.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                No items added. Click &quot;Add item&quot; to add products to the quotation.
              </div>
            ) : (
          ...
          <div className="bg-card border border-border/50 rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-foreground">Discount & Tax</h3>
            </div>
            ...
            <div className="space-y-2">
              ...
              {discountAmount > 0 && (
                <div className="flex justify-between text-rose-600">
                  <span>Discount</span>
                  <span className="font-medium">-{formatCurrency(discountAmount, profile?.currency, profile?.currencyPos as any)}</span>
                </div>
              )}
              {taxAmount > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Tax</span>
                  <span className="font-medium">{formatCurrency(taxAmount, profile?.currency, profile?.currencyPos as any)}</span>
                </div>
              )}
              <div className="flex justify-between pt-2 border-t border-border/50">
                <span className="text-lg font-bold text-foreground">Grand Total</span>
                <span className="text-2xl font-bold text-emerald-600">{formatCurrency(grandTotal, profile?.currency, profile?.currencyPos as any)}</span>
              </div>
            </div>
          </div>
