// src/components/expense/add-expense-modal.tsx
"use client";
...
import { createExpense, getExpenseCategories } from "@/lib/actions/expense";
...
export function AddExpenseModal({
  ...
  useEffect(() => {
    if (open && activeProfile?.id) {
      getExpenseCategories(activeProfile.id).then((res) => {
  ...
  const onSubmit = async (data: CreateExpenseInput) => {
    ...
    const res = await createExpense(activeProfile.id, {
      ...data,
      items: items
  ...
  return (
    <AnimatedModal open={open} onClose={onClose} title="Add Expense">
              <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
                ...
                {/* Category */}
                <div>
                  <Label className="text-xs text-muted-foreground mb-1.5 block">Expense Category</Label>
                  <select
                    {...register("categoryId")}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        defaultValue={itemsTotal > 0 ? itemsTotal : undefined}
                      />
                    </div>
                    {errors.totalAmount && (
